...
void QposHost::foreachPeer(std::function<bool(std::shared_ptr<QposPeer>)> const& _f) const
{
	...
	for (auto s : sessions)
		if (!_f(capabilityFromSession<QposPeer>(*s.first)))
			return;
	...
	std::sort(sessions.begin(), sessions.end(), sessionLess);
	for (auto s : sessions)
		if (!_f(capabilityFromSession<QposPeer>(*s.first, c_oldProtocolVersion)))
			return;
